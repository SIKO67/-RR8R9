    ...
    for x in formats:
        ...
        if type == "audio":
            if "audio" in check:
                j += 1
                if j == 1:
                    a1 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}")
                if j == 2:
                    a2 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}")
                if j == 3:
                    a3 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}") 
                if j == 4:
                    a4 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}")
                if j == 5:
                    a5 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}")
                if j == 6:
                    a6 = InlineKeyboardButton(text=f"صوت بدقة 🎵 {humanbytes(x['filesize'])}", callback_data=f"ytdata audio||{x['format_id']}||{videoid}")  
        elif type == "video":
            if str(133) in check:
                a += 1
                if int(a) == 1:
                    j += 1
                    a += 1
                    a1 = InlineKeyboardButton(text=f"(240)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")
            if str(134) in check:
                b += 1
                if int(b) == 1:
                    j += 1
                    b += 1
                    a2 = InlineKeyboardButton(text=f"(360)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")
            if str(135) in check:
                c += 1
                if int(c) == 1:
                    j += 1
                    c += 1
                    a3 = InlineKeyboardButton(text=f"(480)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")   
            if str(136) in check:
                d += 1
                if int(d) == 1:
                    j += 1
                    d += 1
                    a4 = InlineKeyboardButton(text=f"(720)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")
            if str(137) in check:
                e += 1
                if int(e) == 1:
                    j += 1
                    e += 1
                    a5 = InlineKeyboardButton(text=f"(1080)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")
            if str(313) in check:
                f += 1
                if int(f) == 1:
                    j += 1
                    f += 1
                    a6 = InlineKeyboardButton(text=f"(2160)pفيديو بدقة 📹 {humanbytes(x['filesize'])}", callback_data=f"ytdata video||{x['format_id']}||{videoid}")   
        else:
            return await CallbackQuery.message.reply_text("Video Format Not Found.")
    universal = InlineKeyboardButton(text="مسح •", callback_data=f'close2')
    if j == 0:
        return await CallbackQuery.message.reply_text("Video Format Not Found..")
    elif j == 1:
...
async def ytdata(_,CallbackQuery):  
    ...
    Name = CallbackQuery.from_user.first_name
    if type == 'audio':
        a1 = InlineKeyboardButton(text=f"• صوت", callback_data=f"boom audio||{format}||{videoid}")
        a2 = InlineKeyboardButton(text=f"• نموذج  ", callback_data=f"boom docaudio||{format}||{videoid}")
    else:
        a1 = InlineKeyboardButton(text=f"• فيديو", callback_data=f"boom video||{format}||{videoid}")
        a2 = InlineKeyboardButton(text=f"• نموذج ", callback_data=f"boom docvideo||{format}||{videoid}")
    key = InlineKeyboardMarkup(
            [
                [
                    a1,
...
inl = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="تحميل ❤️‍🔥......", callback_data=f'down')
                ]   
            ]
        )
...
upl = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="تنزيل❤️‍🔥......", callback_data=f'down')
                ]   
            ]
        )
...
async def boom(_,CallbackQuery): 
    ...
    try:
        ...
        await CallbackQuery.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(buttons)) 
    mystic = await CallbackQuery.edit_message_text("يتم التحميل، ارجوك انتضر قليلا.", reply_markup = inl)
    fetched = f"""
**تم التحميل بنجاح**
__حمل اي اغنية مره اخرى 🧑‍💻__ """    
    link = (x["webpage_url"])
...
def p_mark(link, channel):
    buttons= [
            [
                InlineKeyboardButton(text="رابط الاغنية", url=f'{link}')
            ],
            [ 
                InlineKeyboardButton(text="رابط قناة المغني", url=f'{channel}')
            ],
        ]
    return buttons    
    
async def send_file(CallbackQuery, med, filename, videoid, user_id, link, channel):
    await CallbackQuery.edit_message_text("رفع الملف الان\n\nانتضر قليلا، من فضلك ❤️‍🔥..", reply_markup = upl)
    try:
